...
#
#  Проверено живьём 2026-08-13 на Wine 11.15, на чистом префиксе от начала до
#  конца: Razer Axon 2.9.1.0, Razer Central 7.23.0.1220, WebView2 151.0.4129.78.
#  Все итоговые проверки зелёные; повторный запуск на готовом отсеке
#  отрабатывает за 16 секунд. Установленное запускается одной командой и
#  показывает каталог обоев. Разбор каждого шага и все замеры —
#  в docs/установка_под_wine_2_9_1_0.md (заходы 9-16).
#
...
# Перечислить программы Wine, работающие ИМЕННО в нашем отсеке.
#
# Признак «наш отсек» берём из переменной окружения процесса, а не из его имени:
# чужие программы с похожими именами трогать нельзя ни при каких обстоятельствах.
#
# ⚠ Но одной этой проверки мало. Сам скрипт тоже экспортирует WINEPREFIX, поэтому
# по ней он находит СЕБЯ и свои подоболочки — и, дойдя до закрытия отсека, убивает
# сам себя посреди установки. Поэтому второе условие: процесс должен быть именно
# программой Wine. У них либо путь по-виндовому (C:\…), либо это wineserver, либо
# двоичный файл самого Wine.
prefix_pids() {
    local pid owner first
    for pid in $(ls /proc 2>/dev/null | grep -E '^[0-9]+$'); do
        ...
        [ "$owner" = "$WINEPREFIX" ] || continue
        first="$(tr '\0' '\n' 2>/dev/null < "/proc/$pid/cmdline" | head -1)"
        case "$first" in
            [A-Za-z]:\\*|*/wineserver|*/wine|*/wine64|*/wine-preloader|*/wine64-preloader|*/bin/wine*)
                echo "$pid" ;;
        esac
    done
    return 0
...
# =============================================================================
#  Шаг 9. Шрифты для русских и других нелатинских подписей
#
#  Свои подписи Razer рисует фирменным шрифтом RazerF5. В нём 229 знаков —
#  латиница и знаки препинания, и всё. Русских букв в нём нет вовсе.
#
#  На настоящей Windows это не беда: если в шрифте нет нужной буквы, .NET берёт
#  её из запасного шрифта. Список запасных лежит в файле самого .NET
#  (GlobalUserInterface.CompositeFont), и для всей полосы знаков от латиницы до
#  кириллицы там записаны три имени: Segoe UI, Segoe UI Symbol, Ebrima. Это
#  системные шрифты Windows.
#
#  В Wine шрифтов с такими именами нет и быть не может — они не свободные.
#  Обычным программам Windows это не мешает: если попросить у Wine «Segoe UI»
#  напрямую, он молча подставит похожий системный шрифт. А вот .NET просит
#  иначе — он сначала спрашивает, ЕСТЬ ЛИ шрифт с таким именем. Ответ «нет»,
#  и весь список запасных обрывается на первом же шаге: буква не рисуется
#  ничем, на её месте остаётся пустой прямоугольник. Так русские пункты меню
#  значка в трее и превращались в квадратики.
#
#  Лечится штатной возможностью самого Wine: он умеет показывать имеющийся
#  шрифт ещё и под чужим именем. Ниже мы говорим, что «Segoe UI» — это тот
#  шрифт, который в системе действительно есть. Список перечислен через
#  разделитель: Wine берёт первый найденный, поэтому запись годится для любого
#  дистрибутива. Tahoma в конце — она входит в сам Wine и есть всегда.
#
#  Если настоящий шрифт с таким именем в отсеке появится (например, его
#  принесёт какая-нибудь программа), Wine подмену молча отменит — это его
#  собственное правило, ломать нечего.
# =============================================================================

# Чем закрываем латиницу, кириллицу и греческий.
FONT_FALLBACK='Noto Sans|DejaVu Sans|Liberation Sans|Arial|Tahoma'
# Чем закрываем стрелки, галочки и прочие значки.
FONT_FALLBACK_SYM='DejaVu Sans|Noto Sans|Liberation Sans|Arial|Tahoma'

setup_fonts() {
    step "Настраиваю шрифты для русских подписей"
    local name
    for name in "Segoe UI" "Ebrima"; do
        timeout 60 wine reg add 'HKCU\Software\Wine\Fonts\Replacements' \
            /v "$name" /t REG_MULTI_SZ /s '|' /d "$FONT_FALLBACK" /f >/dev/null 2>&1
    done
    timeout 60 wine reg add 'HKCU\Software\Wine\Fonts\Replacements' \
        /v 'Segoe UI Symbol' /t REG_MULTI_SZ /s '|' /d "$FONT_FALLBACK_SYM" /f >/dev/null 2>&1
    close_prefix
    if grep -aqF 'Wine\\Fonts\\Replacements' "$WINEPREFIX/user.reg" 2>/dev/null; then
        note "Шрифты настроены"
    else
        warn "Настройку шрифтов подтвердить не удалось — русские подписи могут остаться квадратиками"
    fi
}

# =============================================================================
#  Шаг 10. Ярлык в меню и команда razer-axon
# =============================================================================
...
# =============================================================================
#  Шаг 12. Проверяем, что всё на месте
# =============================================================================
...
verify() {
    ...
    check "русские подписи не квадратиками"       in_registry user.reg 'Wine\\Fonts\\Replacements'
