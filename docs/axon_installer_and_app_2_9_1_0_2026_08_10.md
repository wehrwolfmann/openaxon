...
6. **Инсталлятор приложения — Inno Setup 6.7.0**, которого не понимает `innoextract`
   (потолок git-master — 6.4). Распаковать по-прежнему НЕ удалось, но из двух
   препятствий **снято одно**: разобран и пропатчен новый заголовок загрузчика
   (ревизия 2, 64-битные смещения), патч лежит в
   `patch/innoextract-inno67-loader-rev2.patch`. Осталось второе — заголовок блока
   настроек. Разбор и план в §8.
